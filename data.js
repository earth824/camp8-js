...
// 1#
const result1 = sales.length;
console.log(result1);

// 2#
const result2 = sales.reduce((acc, item) => {
  const { product : { unitPrice }, customer, discount } = item;
  const total = ((1 - discount) || 1) * unitPrice;
  if (acc[customer]) {
    acc[customer] = {
      count: acc[customer].count + 1,
      total: acc[customer].total + total,
    }
  } else {
    acc[customer] = {
      count: 1,
...
const result3 = sales.reduce((acc, item) => {
  ...
  return acc + ((1 - discount) || 1) * unitPrice;
}, 0);
console.log(result3);
...
const result6 = sales.reduce((acc, item) => {
  const { product : { unitPrice }, discount, type } = item;
